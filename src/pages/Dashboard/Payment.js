...
const Payment = () => {
  ...
  const { data: tool, isLoading } = useQuery(["toolsOrder", id], () =>
    fetch(`https://tranquil-earth-82270.herokuapp.com/order/${id}`).then(
      (res) => res.json()
    )
  );
  if (isLoading) {
    ...
    <div className="bg-accent">
      <div className="h-screen bg-gray-800">
        ...
        <div className="flex flex-col rounded-xl md:flex-row justify-center mt-8 md:mt-20  items-center mx-auto container w-full md:w-5/6 bg-accent py-3 ">
          <div className="card  h-full flex-1 bg-accent  ">
            <div className="card-body ml-0 pl-3 text-white">
              <h2 className="text-white font-koulen uppercase font-semibold text-2xl">
                {" "}
                Welcome <span className="text-white">{name}</span>{" "}
              </h2>
              <h2 className="uppercase font-semibold mt-5  text-white">
                Payment For :{" "}
          ...
          <div className="card   w-full h-full  flex-1 bg-white opacity-80  ">
            <div className="">
